...
var naiveBayesTextClassifier = function() {
  ...
  var logLikelihood = function(w, label) {
    ...
    return config.smoothingFactor > 0
  ...
  var defineConfig = function(cfg) {
    ...
    if (!helpers.object.isObject(cfg)) {
      throw Error(
        "winkNBTC: config must be an object, instead found: " + typeof cfg
      );
    }
    config.considerOnlyPresence =
      typeof cfg.considerOnlyPresence === "boolean"
        ? cfg.considerOnlyPresence
        : false;

    // If smoothing factor is undefined set it to lapalce add+1 smoothing.
    var sf =
      cfg.smoothingFactor === undefined ? 1 : parseFloat(cfg.smoothingFactor);
    // Throw error for a value beyond 0-1 or NaN.
    if (isNaN(sf) || sf < 0 || sf > 1) {
      throw Error(
        "winkNBTC: smoothing factor must be a number between 0 & 1, instead found: " +
          JSON.stringify(sf)
      );
    }
  ...
  var definePrepTasks = function(tasks) {
    if (!helpers.array.isArray(tasks)) {
      throw Error(
        "winkNBTC: tasks should be an array, instead found: " +
    ...
    for (var i = 0, imax = tasks.length; i < imax; i += 1) {
      if (typeof tasks[i] !== "function") {
        throw Error(
          "winkNBTC: each task should be a function, instead found: " +
  ...
  var consolidate = function() {
    ...
    if (labelCount < 2) {
      throw Error(
        "winkNBTC: can not consolidate as classification require 2 or more labels!"
      );
    }
    if (voc.size < 10) {
      throw Error(
        "winkNBTC: vocabulary is too small to learn meaningful classification!"
      );
    }
  ...
  var computeOdds = function(input) {
    ...
    if (!consolidated) {
      throw Error(
        "winkNBTC: prediction is not possible unless learnings are consolidated!"
      );
    }
  ...
  var evaluate = function(input, label) {
    ...
    if (!samples[label]) {
      throw Error(
        "winkNBTC: can not evaluate, unknown label enountered: " +
